      ...
      <section className="section-padding bg-dark-DEFAULT">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap gap-3 mb-12">
            {categories.map((cat) => (
              <button key={cat} className={`px-5 py-2 text-sm font-semibold uppercase tracking-wider border transition-all duration-300 ${cat === 'All' ? 'bg-brand-red border-brand-red text-white' : 'border-white/10 text-gray-400 hover:border-brand-red hover:text-brand-red'}`}>
                {cat}
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {(posts.length > 1 ? posts.slice(1) : posts).map((post: any) => (
              <Link key={post.slug} href={`/blog/${post.slug}`} className="group bg-dark-100 border border-white/5 hover:border-brand-red/30 transition-all duration-500 overflow-hidden flex flex-col">
                <div className="relative h-52 overflow-hidden">
                  <Image src={post.image} alt={post.title} fill sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-cover transition-transform duration-700 group-hover:scale-110" />
